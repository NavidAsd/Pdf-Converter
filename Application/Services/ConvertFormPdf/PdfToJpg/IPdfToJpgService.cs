...
namespace Application.Services.ConvertFormPdf.PdfToJpg
{
    public interface IPdfToJpgService
    {
        ResultMessage<ReturnPdfToImagesDto> Execute(RequestPdfToJpgDto request);
    }
    public class RequestPdfToJpgDto
    {
        public string OutputPath { set; get; }
        public string InputPath { set; get; }
        public string UserIp { set; get; }
    }
    public class PdfToJpgService : IPdfToJpgService
    {
        ResultMessage<ReturnPdfToImagesDto> IPdfToJpgService.Execute(RequestPdfToJpgDto request)
        {
            try
            {
                ...
                using (var document = PdfiumViewer.PdfDocument.Load(request.InputPath))
                {
                    ...
                    for (int i = 0; i < document.PageCount; i++)
                    {
                        ...
                        image.Save($"{ImagePath}\\{ImageName}", ImageFormat.Jpeg);
                        files[i] = $"{ImagePath}\\{ImageName}";
                    }
                }
                string OutFileName = Formating.ReturnZipFileName(FilesFormat.zip, "Jpg");
